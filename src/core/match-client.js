...
export class MatchClient {
 #events; #room=null; #unsubscribe=null; #user=null; #matchUnsubscribe=null; #gameUnsubscribe=null; #gameHandlers=new Set(); #seq=0; #lastGameSyncAt=0; #gameSyncBusy=false; #roomPoll=null; #heartbeat=null; #lastOpponentConnected=null; #disconnectTimer=null;
 ...
 #watch(roomId){
  this.#unsubscribe?.();
  const refresh=async()=>{if(!this.#room||this.#room.id!==roomId)return;try{const [room,players]=await Promise.all([db.getRow({databaseId:APPWRITE.databaseId,tableId:"rooms",rowId:roomId}),this.#players(roomId)]);this.#room=this.#map(room,players);this.#events.emit("match:update",this.room);const opponent=this.#room.players.find(p=>p.userId!==this.#user?.$id);if(opponent){if(this.#lastOpponentConnected===true&&opponent.connected===false){clearTimeout(this.#disconnectTimer);this.#events.emit("match:opponent-disconnected",{room:this.room,opponent,graceMs:8000});this.#disconnectTimer=setTimeout(async()=>{if(!this.#room)return;const latest=(await this.#players(this.#room.id)).find(p=>p.user_id===opponent.userId);if(latest?.connected===false)this.#events.emit("match:opponent-left",{room:this.room,opponent,reason:"disconnect"});},8000);}if(opponent.connected!==false&&this.#lastOpponentConnected===false){clearTimeout(this.#disconnectTimer);this.#events.emit("match:opponent-reconnected",{room:this.room,opponent});}this.#lastOpponentConnected=opponent.connected!==false;}if(room.status==="countdown"||room.status==="playing"){const match=await this.watchCurrentMatch();if(match)this.#events.emit("match:state",match);}}catch(e){console.warn("Lobby refresh failed",e?.message||e)}};
  this.#unsubscribe=client.subscribe([`tablesdb.${APPWRITE.databaseId}.tables.room_players.rows`],refresh);
  setTimeout(refresh,250);clearInterval(this.#roomPoll);this.#roomPoll=setInterval(refresh,1200);this.#startPresence();
 }
 async #setPresence(connected){const user=await this.#auth();if(!this.#room)return;const rows=await db.listRows({databaseId:APPWRITE.databaseId,tableId:"room_players",queries:[Query.equal("room_id",[this.#room.id]),Query.equal("user_id",[user.$id]),Query.limit(1)]});const row=rows.rows?.[0];if(row)await db.updateRow({databaseId:APPWRITE.databaseId,tableId:"room_players",rowId:row.$id,data:{connected}});}
 #startPresence(){clearInterval(this.#heartbeat);this.#heartbeat=setInterval(()=>{if(document.visibilityState==="visible")this.#setPresence(true).catch(()=>{});},10000);}
 async leaveRoom(){const user=await this.#auth();if(!this.#room)return;const rows=await db.listRows({databaseId:APPWRITE.databaseId,tableId:"room_players",queries:[Query.equal("room_id",[this.#room.id]),Query.equal("user_id",[user.$id]),Query.limit(1)]});const row=rows.rows?.[0];if(row)await db.updateRow({databaseId:APPWRITE.databaseId,tableId:"room_players",rowId:row.$id,data:{connected:false,ready:false}});clearInterval(this.#heartbeat);this.#heartbeat=null;this.#events.emit("match:player-left",{userId:user.$id,room:this.room});this.leave();}
 ...
 leave(){const room=this.room;clearTimeout(this.#disconnectTimer);this.#disconnectTimer=null;clearInterval(this.#heartbeat);this.#heartbeat=null;this.#lastOpponentConnected=null;clearInterval(this.#roomPoll);this.#roomPoll=null;this.#unsubscribe?.();this.#matchUnsubscribe?.();this.#gameUnsubscribe?.();this.#unsubscribe=null;this.#matchUnsubscribe=null;this.#gameUnsubscribe=null;this.#gameHandlers.clear();this.#room=null;this.#events.emit("match:left",room);}
 realtimeAdapter(matchId){const ensure=async()=>{const user=await this.#auth();if(!this.#gameUnsubscribe){this.#gameUnsubscribe=client.subscribe([`tablesdb.${APPWRITE.databaseId}.tables.game_events.rows`],response=>{const row=response?.payload||response;if(row?.match_id!==matchId||row?.sender_id===this.#user?.$id)return;try{const msg=JSON.parse(row.payload);this.#gameHandlers.forEach(fn=>fn({...msg,sender:row.sender_id}))}catch{}});}return user;};ensure();return{send:async payload=>{const user=await ensure();const isSync=payload.event==="TANK_SYNC",now=Date.now();if(isSync&&(this.#gameSyncBusy||now-this.#lastGameSyncAt<120))return;if(isSync){this.#gameSyncBusy=true;this.#lastGameSyncAt=now;}const data={...payload};delete data.sender;try{await db.createRow({databaseId:APPWRITE.databaseId,tableId:"game_events",rowId:ID.unique(),data:{match_id:matchId,sender_id:user.$id,event:String(payload.event||"GAME").slice(0,32),payload:JSON.stringify(data).slice(0,4096),seq:++this.#seq},permissions:[Permission.read(Role.users()),Permission.delete(Role.user(user.$id))]});}catch(e){if(!/rate limit/i.test(String(e?.message||"")))console.warn("Game realtime send failed",e?.message||e);}finally{if(isSync)this.#gameSyncBusy=false;}},on:(name,fn)=>{if(name==="message")this.#gameHandlers.add(fn);return()=>this.#gameHandlers.delete(fn)},off:(name,fn)=>{if(name==="message")this.#gameHandlers.delete(fn)},close:()=>{this.#gameUnsubscribe?.();this.#gameUnsubscribe=null;this.#gameHandlers.clear();}}}
