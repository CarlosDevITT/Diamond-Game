...
export class MatchClient {
 ...
 async submitResult({matchId,score,survivalMs}){const user=await this.#auth();const row=await db.getRow({databaseId:APPWRITE.databaseId,tableId:"matches",rowId:matchId});const slot=row.player_a===user.$id?"A":row.player_b===user.$id?"B":null;if(!slot)throw new Error("Jogador não pertence a esta partida.");const data=slot==="A"?{score_a:Math.max(0,Number(score)||0),survival_a_ms:Math.max(0,Number(survivalMs)||0)}:{score_b:Math.max(0,Number(score)||0),survival_b_ms:Math.max(0,Number(survivalMs)||0)};await db.updateRow({databaseId:APPWRITE.databaseId,tableId:"matches",rowId:matchId,data});this.#events.emit("match:result-submitted",{slot,...data});return slot;}
