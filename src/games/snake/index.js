...
export const snakeGame={
  ...
  create({root,close}){
    ...
    const cells=18,scores=new ScoreStore(); let body,food,dir,next,score,best=scores.getBest("snake"),timer=null,touch=null;
    ...
    const tick=()=>{const h={x:body[0].x+next.x,y:body[0].y+next.y};dir=next;if(h.x<0||h.y<0||h.x>=cells||h.y>=cells||body.some(p=>p.x===h.x&&p.y===h.y)){best=scores.saveBest("snake",score);bestNode.textContent=best;reset();return}body.unshift(h);if(h.x===food.x&&h.y===food.y){score+=10;scoreNode.textContent=score;spawn()}else body.pop();draw()};
    ...
    return {start(){if(!timer)timer=setInterval(tick,115)},destroy(){clearInterval(timer);timer=null;window.removeEventListener("keydown",keydown);window.removeEventListener("resize",resize)}};
  }
};
