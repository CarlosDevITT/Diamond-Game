export class TankControls{
 ...
 start(){
  ...
  const bind=(el,type)=>{let id=null,cx=0,cy=0;const move=e=>{if(e.pointerId!==id)return;const dx=e.clientX-cx,dy=e.clientY-cy;if(type==="move"){const dead=7,range=54;this.touchMove=Math.abs(dy)<dead?0:Math.max(-1,Math.min(1,-dy/range));this.touchTurn=Math.abs(dx)<dead?0:Math.max(-1,Math.min(1,dx/range))}else this.#turret=Math.atan2(dy,dx);el.querySelector("i").style.transform=`translate(${Math.max(-38,Math.min(38,dx))}px,${Math.max(-38,Math.min(38,dy))}px)`};on(el,"pointerdown",e=>{id=e.pointerId;const r=el.getBoundingClientRect();cx=r.left+r.width/2;cy=r.top+r.height/2;el.setPointerCapture(id);move(e)});on(el,"pointermove",move);on(el,"pointerup",()=>{id=null;if(type==="move"){this.touchMove=0;this.touchTurn=0}el.querySelector("i").style.transform=""})};
