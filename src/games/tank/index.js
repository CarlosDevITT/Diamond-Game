import { TankControls } from "./tank-controls.js?v=20260920-50";
...
export const tankGame={
 ...
 create({root,close,options={}}){
  ...
  const casual=options.mode!=="1v1";const difficulty=["basic","hard","pro"].includes(options.difficulty)?options.difficulty:"basic";const difficultyLabel={basic:"BÁSICO",hard:"HARD",pro:"PRO"}[difficulty];const bus=options.eventBus,realtime=options.realtimeClient,matchContext={...(options.matchContext||options),casual,difficulty};matchContext.slot=options.slot||matchContext.slot;if(casual)opponentLabel.textContent=`MÁQUINA • ${difficultyLabel}`;
