...
export const tankGame={
 ...
 create({root,close,options={}}){
  ...
  let timer=null,ended=false;const parsedStart=Date.parse(options.startedAt);let startAt=Number.isFinite(parsedStart)?parsedStart:Date.now()+3000;if(startAt-Date.now()>7000||Date.now()-startAt>5000)startAt=Date.now()+3000;let matchStart=0;
