import { TankControls } from "./tank-controls.js?v=20260920-30";
import { TankEngine } from "./tank-engine.js?v=20260920-30";
import { buildTankMatchPayload,resolveTankWinner } from "./tank-rules.js?v=20260920-30";
...
export const tankGame={
 ...
 create({root,close,options={}}){
  ...
  const bus=options.eventBus,realtime=options.realtimeClient,matchContext=options.matchContext||options;matchContext.slot=options.slot||matchContext.slot;
  ...
  let timer=null,ended=false;const parsedStart=Date.parse(options.startedAt);let startAt=Number.isFinite(parsedStart)?parsedStart:Date.now()+3000;if(startAt-Date.now()>7000)startAt=Date.now()+3000;let matchStart=startAt;
