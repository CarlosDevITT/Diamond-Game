...
export function drawTankArena({ctx,canvas,world,view,camera,local,remote,projectiles=[],pickups=[],effects=[],showLocal=true,showRemote=true}){
 ...
 if(showRemote&&remote?.alive!==false)drawTankSprite(ctx,remote,"#ff49c7");if(showLocal&&local?.alive!==false)drawTankSprite(ctx,local,"#52ffd2");
 ...
 const mw=150,mh=78,mx=canvas.clientWidth-mw-14,my=14;ctx.fillStyle="rgba(5,7,18,.82)";ctx.fillRect(mx,my,mw,mh);ctx.strokeStyle="#ffffff22";ctx.strokeRect(mx,my,mw,mh);const mapX=x=>mx+x/w*mw,mapY=y=>my+y/h*mh;ctx.fillStyle="#52ffd2";ctx.beginPath();ctx.arc(mapX(local.x),mapY(local.y),4,0,Math.PI*2);ctx.fill();ctx.fillStyle="#ff49c7";ctx.beginPath();ctx.arc(mapX(remote.x),mapY(remote.y),4,0,Math.PI*2);ctx.fill();ctx.strokeStyle="#ffffff44";ctx.strokeRect(mapX(camera.x),mapY(camera.y),vw/w*mw,vh/h*mh);ctx.restore();
}
