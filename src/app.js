...
import { GAME_CATALOG } from "./core/game-catalog.js?v=20260925-96";
...
let match=null,auth=null,authScreen=null,lobby=null,profilePanel=null,pendingGame=null,activeMatchId=null,activeGameId=null,activeDurationMs=null,resultShownFor=null;
...
const startGame=async(id,options={})=>{
 ...
 },{...options,eventBus:events,roomId:match?.room?.id,matchContext:{matchId:options.matchId,roomId:match?.room?.id,seed:options.seed,startedAt:options.startedAt,durationMs:options.durationMs,playerId:match?.userId,slot:match?.room?.players?.find(p=>p.userId===match?.userId)?.slot},slot:match?.room?.players?.find(p=>p.userId===match?.userId)?.slot});
};
...
// Offline support v52
if("serviceWorker" in navigator){window.addEventListener("load",async()=>{try{const reg=await navigator.serviceWorker.register("./sw.js?v=96");await reg.update()}catch(error){console.warn("Offline support unavailable",error)}})}
